...
program.name("gotra").description("CLI Scaffolding Generator untuk Arsitektur Base React Vite").version("1.1.0");
...
program.command("make:module").description("Generate modular feature UI, TanStack Query hooks, schemas, dan inject route").action(async () => {
...
program.command("make:page").alias("make:feature").description("Generate hanya UI feature layer (pages + components) & inject route").action(async () => {
...
program.command("make:service").description("Generate hanya data/API layer (hooks, response, schema)").action(async () => {
...
program.command("make:component").description("Generate shared UI component di src/shared/components/").action(async () => {
  ...
  const targetDir = subPath ? path.resolve(process.cwd(), "src/shared/components", subPath) : path.resolve(process.cwd(), "src/shared/components");
...
program.command("make:hook").description("Generate custom React hook di src/shared/hooks/").action(async () => {
